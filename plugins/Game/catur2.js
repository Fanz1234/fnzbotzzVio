...
const handler = async (m, { conn, args }) => {
  ...
  if (feature === 'delete') {
    ...
    return conn.reply(m.chat, ' *Permainan catur dihentikan.*', m);
  }
  if (feature === 'create') {
    if (!gameData) {
      ...
      return conn.reply(m.chat, ' *Permainan sudah dimulai.*', m);
    }
  } else if (feature === 'join') {
    const senderId = m.sender;
    const joinedPlayers = gameData?.players || [];
    if (joinedPlayers.includes(senderId)) {
      return conn.reply(m.chat, ' *Anda sudah bergabung dalam permainan ini.*', m);
    }
    if (!gameData || gameData.status !== 'waiting') {
      return conn.reply(m.chat, ' *Tidak ada permainan catur yang sedang menunggu.*', m);
    }
    ...
    if (gameData.players.length === 2) {
      ...
      return conn.reply(m.chat, ' *Anda telah bergabung dalam permainan catur.*\nMenunggu pemain lain untuk bergabung.', m);
    }
  } else if (feature === 'start') {
    if (gameData.status !== 'ready') {
      return conn.reply(m.chat, ' *Tidak dapat memulai permainan. Tunggu hingga dua pemain bergabung.*', m);
    }
    ...
    if (!gameData || gameData.status !== 'playing') {
      return conn.reply(m.chat, ' *Permainan belum dimulai.*', m);
    }
    ...
    if (chess.isCheckmate()) {
      return conn.reply(m.chat, ' *Checkmate.*', m);
    }
    if (chess.isDraw()) {
      return conn.reply(m.chat, ' *Draw.*', m);
    }
    if (chess.isGameOver()) {
      return conn.reply(m.chat, ' *Game Over.*', m);
    }
    const [from, to] = args;
    try {
    ...
    // Delete the previous chessboard message
  if (chessData.boardMessageId) {
    conn.deleteMessage(m.chat, chessData.boardMessageId);
  }

  // Store the message ID of the current chessboard
  const sentMessage = await conn.sendFile(m.chat, boardUrl, '', giliran, m, false, { mentions: [chessData.currentTurn] });
  chessData.boardMessageId = sentMessage?.message?.ephemeral_id;
