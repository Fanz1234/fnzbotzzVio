...
let handler = async (m, { conn, text, command }) => {
    ...
    const nazz_cmd = {
        create: () => {
            ...
            ut[m.chat] = { date: Date.now(), status: 'WAITING', host: m.sender, players: {}, map: anu.map, map_name: anu.name, ular_tangga: anu.nazz, stabil_x: anu.stabil_x, stabil_y: anu.stabil_y, msg: {} };
...
async function kocok(m, ut, conn) {
        if(!ut[m.chat]) return;
        try { await conn.sendMessage(m.chat, { delete: ut[m.chat].msg }) } catch(e) {}
        const players = Object.keys(ut[m.chat].players);
       ...
       let key = await conn.sendMessage(m.chat, { sticker: { url: `https://raw.githubusercontent.com/fgmods/fg-team/main/games/dados/${dadu}.webp` }, packname: "Created by Zippo", author: "Naaazzzzz" }, { quoted: m })
        ut[m.chat].turn += 1;
        ...
        let msgs = await conn.sendMessage(m.chat, { image: await drawBoard(ut[m.chat].map, user1, user2, user3, user4, ut[m.chat].stabil_x, ut[m.chat].stabil_y), caption: `${warna} *${langkah}* --> *${ut[m.chat].players[m.sender].langkah}*${teks ? teks:""}\nMenunggu @${players[ut[m.chat].turn %= players.length].split("@")[0]} mengetik *kocok*`, mentions: [players[ut[m.chat].turn %= players.length]]}, { quoted: key });
        
        return ut[m.chat].msg = msgs.key
    }
...
Menunggu @${players[0].split("@")[0]} mengetik *kocok*`;
   let msgs = await conn.sendMessage(m.chat, { image: await drawBoard(ut[m.chat].map, 1, null, null, null, ut[m.chat].stabil_x, ut[m.chat].stabil_y), caption: teks, mentions: conn.parseMention(teks) }, { quoted: m })
    ut[m.chat].turn = 0
    ut[m.chat].msg = msgs.key
}
